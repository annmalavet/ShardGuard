...
def _normalize_json_schema(schema: Any) -> dict[str, Any]:
    ...
    # Optional fields must be nullable so OpenAI strict mode accepts null
    props = {}
    ...
    def refresh(self) -> None:
        ...
        for server_name, tools in tools_by_server.items():
            ...
            for t in tools or []:
                ...
                logger.debug(
                    f"[catalog] registered tool={name!r} server={server_name!r}\n"
...
def _resolve_placeholders(value: Any, secrets: dict[str, str]) -> Any:
    """Recursively replace placeholders with their secret values."""
...
def _contains_concat_placeholders(s: str) -> bool:
    """Detect obvious placeholder concatenation."""
...
class CoordinationService:
    ...
    def _visible_for_step(
        ...
        *,
        ...
        # Include keys from placeholder_args mapping
    ...
    @staticmethod
    def _prefilled_args_from_step(step: dict[str, Any]) -> dict[str, Any]:
        """Build pre-filled args from planner's placeholder_args mapping.

        placeholder_args: {"SECRET_1": "pattern"} → {"pattern": "$[SECRET_1]"}
        """
        mapping: dict[str, str] = step.get("placeholder_args") or {}
        return {arg_name: f"$[{ph_key}]" for ph_key, arg_name in mapping.items()}
    ...
    async def _validate_and_resolve_args(
        ...
        *,
        ...
        """
        Returns (resolved, canon, resolved_pre_coerce, error). On success error is None; on failure the first three are None.
        """
        ...
        _write_context_log(
            {
                ...
                "context": {
                    ...
                    "user": subprompt_result,
                },
                "estimated_chars": len(self.system_prompt) + len(subprompt_result),
            }
        )
    ...
    async def _run_planned_steps(
        ...
        *,
        ...
        for idx, step in enumerate(steps):
            ...
            prefilled = self._prefilled_args_from_step(step)
            ...
            # Planner's explicit placeholder→arg mappings take precedence over LLM output
            parsed_args = {**parsed_args, **prefilled}
    ...
    async def runJob(
        ...
        # 1. Establish the Unique ID for this Loop
        self._run_id = str(uuid.uuid4())
        self._prompt_id = str(uuid.uuid4())
        llm_call_count = 0

        # 2. Log the Top Level "Job Start" Event
        ...
        if allowed_tools is None:
            ...
            planning = await self._planner.plan(redacted_prompt, self._tool_summaries())
        ...
        if execute is False:
            return {
                "run_id": self._run_id,  # Return ID so caller can match events
                "planning": planning,
            }
